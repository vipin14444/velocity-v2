...
import { LISTENER, MOD, SPEAKER } from "./DailyAudio";
import {
    useCallback,
    useEffect,
    useState,
...
export const CallProvider = ({ children }) => {
    ...
    const handleParticipantJoinedOrUpdated = useCallback((evt) => {
        setUpdateParticipants(
            `updated-${evt?.participant?.user_id}-${Date.now()}`
        );
    ...
    const handleParticipantLeft = useCallback((evt) => {
        setUpdateParticipants(
            `left-${evt?.participant?.user_id}-${Date.now()}`
        );
    ...
    const updateUsername = useCallback(
        (newAccountType) => {
             ...
             */
            const split = callFrame
                ?.participants()
                ?.local?.user_name.split("✋ ");
